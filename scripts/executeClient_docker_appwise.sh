...
id=0
APPS=('alexnet' 'resnet50' 'vgg16'  'mnasnet' 'mobilenet_v3' 'efficientnet')
rate_apps=(200 150 80 200 250 250)
flag=(1 0 0 1 0 1)

nclient=1
request=$(($EXPERIMENT_TIME * 10))
...
do
	...
	$BUILD_DIR/client_1 --task $app --hostname $ADDR  --portno 8080 \
        --requests $request --batch 1 --rate ${rate_apps[i]} --change_flag ${flag[i]} \
        --input $RESOURCE_DIR/$input_txt --skip_resize 1 --root_data_dir $DATA_ROOT_DIR \
        --dist uni --flux 0 1>$RESULT_DIR/$app-$num-client.csv &
