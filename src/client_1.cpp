...
#define CHANGE_INTERVAL 10 //interval of changing generate rate
#define TOTAL_TIME 20 //total time of generating 5min

namespace po = boost::program_options; 

std::map <std::string, int> max_rate={{"alexnet",350}, {"resnet50",250},{"vgg16",150},{"resnet18",250},{"vgg19",120},{"mnasnet",350},{"mobilenet_v3",250},{"efficientnet",300}};
std::map <std::string, int> min_rate={{"alexnet",100}, {"resnet50",100},{"vgg16",50},{"resnet18",100},{"vgg19",50},{"mnasnet",100},{"mobilenet_v3",100},{"efficientnet",100}};
...
po::variables_map parse_opts(int ac, char** av) {
	...
	desc.add_options()("help,h", "Produce help message")
		...
		("change_flag,c,",po::value<int>()->default_value(0)," change flag of generate rate (+/- 10)")
...
torch::Tensor getInput(const char* net_name){
	...
	else if(strcmp(net_name,"resnet50")==0 ||strcmp(net_name,"resnet18")==0 ||strcmp(net_name,"vgg16")==0||strcmp(net_name,"vgg19")==0 ||strcmp(net_name,"alexnet")==0||strcmp(net_name,"mnasnet")==0||strcmp(net_name,"mobilenet_v3")==0||strcmp(net_name,"efficientnet")==0){
...
void setupGlobalVars(po::variables_map &vm){
	...
	g_changeflag = vm["change_flag"].as<int>();
}
...
#endif
#ifdef DEBUG
			std::cout <<"sending " << i+1 << " as tid to server "<<std::endl;
#endif
...
void *sendRequest(void *vp){
	...
	start_per = getCurNs();
	int change_count = 0;
	while(change_count < TOTAL_TIME){
	//for (uint64_t i =0; i < g_numReqs; i++){
		end_per = getCurNs();
		if (double(end_per-start_per)/1000000000 >= CHANGE_INTERVAL){
			//printf("total_time: %.5lf, ", double(end_t-start_t)/1000000000);
			printf("change_time: %.5lf,change_count:%d\n", double(end_per-start_per)/1000000000,change_count);
			change_count +=1;
			if (g_rate == min_rate[g_model]|| g_rate == max_rate[g_model]){
				g_changeflag += 1;
			}
			if (g_changeflag % 2 == 1){
				g_rate += 10;
...
int main(int argc, char** argv) {
	    ...
	    std::cout << (char*)g_hostName.c_str() <<":"<<g_portNo << std::endl;
