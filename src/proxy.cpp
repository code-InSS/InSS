...
void warmupModel(int id, int max_batch_size, torch::Device &gpu_dev)
{
	...
	torch::Tensor input = getRandInput(id, 1);
#ifdef PROXY_LOG
...
void* recvInput(void* vp){
	...
	while (1){
		...
		while(1){
			...
			else if(dimlen == END_PROXY){
				gp_proxyCtrl->markProxy(gp_PInfo, EXITING);
			}
			else{
...
void* compute(void* vp){
	...
	while(1){
		...
		inputs.push_back(t);
		{
			...
			torch::Tensor temp;
			if(temp_output.isTuple()){
				temp = temp_output.toTuple()->elements()[0].toTensor(); // 1st output;
				temp = temp.to(torch::Device(torch::kCPU));
				//std::cout << t.sizes()<< std::endl;
			}
			else{ temp = temp_output.toTensor().to(torch::Device(torch::kCPU));
			//std::cout << t.sizes()<< std::endl;
			}
			/*
