...
void LatencyModel::setupTable_new(std::string TableFile){
    std::string str_buf;
    std::ifstream file(TableFile);
    std::string line;
    #ifdef DEBUG
    ...
    while(std::getline(file, line)){
        ...
        std::string model=field;
        /*
		std::map<std::string,std::unordered_map<int,float>*>::iterator it = _perModelKernels.find(model);
		if(it == _perModelLatnecyTable.end()){
			_perModelLatnecyTable[model]=new std::unordered_map<int,float>();
            _perModelGPURatioTable[model]=new std::unordered_map<int,float>();
		}
        */
	    std::getline(iss, field,',');
        int kernel = stoi(field);
        std::getline(iss, field,',');
        float baseidle=stoi(field);
        std::getline(iss, field,',');
        float cache=stof(field);
        std::getline(iss, field,',');
        float utilalpha=stof(field);
        std::getline(iss, field,',');
        float utilbeta=stof(field);
        std::getline(iss, field,',');
        float run1=stof(field);
        std::getline(iss, field,',');
        float run2=stof(field);
        std::getline(iss, field,',');
        float run3=stof(field);
        std::getline(iss, field,',');
        float run4=stof(field);
        std::getline(iss, field,',');
        float run5=stof(field);
        _perModelKernels[model] = kernel;
        _perModelBaseidle[model] = baseidle;
        _perModelCache[model]=cache;
        _perModelUtilAlpha[model]=utilalpha;
        _perModelUtilBeta[model]=utilbeta;
        _perModelRun1[model]=run1;
        _perModelRun2[model]=run2;
        _perModelRun3[model]=run3;
        _perModelRun4[model]=run4;
        _perModelRun5[model]=run5;
...
float LatencyModel::getActivetime(std::string model, int batch, int part){
    return (_perModelRun1[model]*batch*batch + _perModelRun2[model]*batch +  _perModelRun3[model])/(part + _perModelRun4[model]) +_perModelRun5[model];
}

int LatencyModel::getkernels(std::string model){
    return _perModelKernels[model];
}

float LatencyModel::getLatency_new(std::string model, int batch, int part){
    assert(MIN_BATCH <= batch && batch <= MAX_BATCH);
    float idle_time=_perModelBaseidle[model];
    float act=getActivetime(model,batch,part);
    return idle_time+act;
}

float LatencyModel::getInterference(std::string my_model,  int my_batch, int my_partition,std::string your_model, int your_partition){
    float my_act=getActivetime(my_model,my_batch,my_partition);
   // float your_act=getActivetime(your_model,your_batch,your_batch);
    float cache_util=_perModelUtilAlpha[your_model]*(your_partition)+_perModelUtilBeta[your_model];
    float interference=my_act*_perModelCache[my_model]*cache_util;
    return interference;
}
