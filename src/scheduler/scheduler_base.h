...
//		bool SquishyBinPacking(std::vector<Task> *task_list, SimState &output);
		...
		float getLatency_new(std::string device, int model_num, int batch, int part); // used for non-interference version of getting latency
		float getLatency_new(std::string device, int model_num, int batch, NodePtr self_node, SimState &input); // used for getting latency also considering interference
	protected:
		...
		int setupDevicePerfModel_new(std::string devcice_config_json_file, std::string res_dir);
		...
		float getInterference_new(std::string device, int a_id, int b_id, int a_batch, int b_batch, int partition_a, int partitoin_b);
		float getInterference_new(std::string device, int model_id, int batch_size, NodePtr node_ptr, GPUPtr gpu_ptr);
		...
		int getNewBatch(Task &self_task, const NodePtr &self_node, SimState &input, int &req,int &workload,int batch,bool is_residue, bool interference);
		...
		//std::vector<std::string> _IDtoModelName = {"resnet50","vgg16", "googlenet", "alexnet",
		//									"inception_v3","densenet", "mobilenet_v3", "squeezenet"};
		
		std::vector<std::string> _IDtoModelName = {"alexnet","resnet50", "vgg16", "resnet18",
											"vgg19","mnasnet", "mobilenet_v3", "efficientnet"};
