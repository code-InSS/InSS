...
namespace Scheduling{
	...
	*/
	...
	bool BaseScheduler::initializeScheduler(std::string sim_config_json_file, \
		...
		if(setupDevicePerfModel_new(device_config_json_file,res_dir)){
	...
	int BaseScheduler::setupScheduler(std::string sim_config_json_file){
		...
		for(auto gpu : root["GPUs"]){
			...
			std::cout << "GPU_type:" << gpu["Type"].asString()<<" ,  num:"<<gpu["Num"].asInt()<<std::endl;     
		}
	...
	int BaseScheduler::setupDevicePerfModel(std::string device_config_json_file, std::string res_dir){
		...
		for (auto dev_spec : root["device_specs"])
		{
			...
			int bandwidth = dev_spec["bandwidth"].as<int>();
			float alpha=dev_spec["alpha_sch"].asFloat();
			float beta=dev_spec["beta_sch"].asFloat();
			...
			_nametoDevPerfModelTable[device].setup(latency_config_json_file,int_model_constant_file,int_util_file,dev_mem,bandwidth,alpha,beta);
#ifdef SCHED_DEBUG
	...
	int BaseScheduler::setupDevicePerfModel_new(std::string device_config_json_file, std::string res_dir){
#ifdef SCHED_DEBUG
		std::cout << __func__ << ": called for " << device_config_json_file << std::endl;
#endif
		Json::Value root;
		...
		for (auto dev_spec : root["device_specs"])
		{
			...
			float beta=dev_spec["beta_sch"].asFloat();
			std::string latency_config_json_file = res_dir +"/"+dev_spec["latency_prof_file"].asString();
			std::string latency_config_json_file_1 = res_dir +"/"+dev_spec["latency_prof_file_1"].asString();
			//std::string int_model_constant_file = res_dir +"/"+dev_spec["interference_const_file"].asString();;
			//std::string int_util_file = res_dir +"/"+dev_spec["interference_util_file"].asString();
			_nametoDevPerfModelTable[device].setup_new(latency_config_json_file,latency_config_json_file_1, dev_mem,bandwidth,alpha,beta);
#ifdef SCHED_DEBUG
			std::cout << "device : "<< device <<" memory: " <<_nametoDevPerfModelTable[device].getDevMem() << std::endl;
#endif
 ...
 void BaseScheduler::setupTasks(std::string task_csv_file, std::vector<Task> *task_list){
                ...
                {
                        std::cout << iter->id <<_IDtoModelName[iter->id] << std::endl;
						std::cout << iter->id << " : [" << iter->request_rate << ", " << iter->SLO << "]" << std::endl;
                        iter->SLO= iter->SLO;
                        std::cout << "configured SLO: "<< iter->SLO <<std::endl;
                }
                std::cout << std::endl;
#endif
	...
	void BaseScheduler::initiateDevs(SimState &input, int nDevs){
		...
		for(auto type : _typeToNumofTypeTable){
			...
			//std::cout << "GPU_type" << gpu_type<<" ,  num:"<<num_of_gpu<<std::endl;
	...
	void BaseScheduler::initDevMems(SimState &input){
		...
		for(auto type_num_pair : _typeToNumofTypeTable){
			...
			//std::cout << "GPU_type" << type<<" ,  ndevs:"<<ndevs<<std::endl;
	...
	float BaseScheduler::getInterference_new(std::string device, int a_id, int b_id, int a_batch, int b_batch,int partition_a, int partitoin_b){
		assert(!device.empty());
		std::string model_a= _IDtoModelName[a_id];
		std::string model_b = _IDtoModelName[b_id];
		float interference=_nametoDevPerfModelTable[device].getInterference_new(model_a,a_batch,partition_a, model_b,partitoin_b);
		return interference;
	}

	float BaseScheduler::getInterference_new(std::string device, const int model_id,const int batch_size,const NodePtr node_ptr,const GPUPtr gpu_ptr){
		assert(!device.empty());
		int count=1;
		float interf=0.0;
		for (auto node: gpu_ptr->vNodeList){
			...
			for(auto task : node->vTaskList){
				count=count+1;
				float interference = getInterference_new(device, model_id,task->id,batch_size,task->batch_size,node_ptr->resource_pntg,node->resource_pntg);
				interf=interf+interference;
			}
		}
		std::string model_name=_IDtoModelName[model_id];
		if(count>1){
			interf=interf+_nametoDevPerfModelTable[device].getInterferenceIdle(model_name,count);
		}
		return interf;
	}
	...
	float BaseScheduler::getLatency_new(std::string device, int model_num, int batch, int part){
		assert(!device.empty());
		assert(1<= batch && batch <= _MAX_BATCH);
		float latency = _nametoDevPerfModelTable[device].getLatency_new(_IDtoModelName[model_num], batch, part);
		//if(_useBatchingOverhead) latency += getBatchLatency(_IDtoModelName[model_num],batch);
		//float lat_ratio=_latencyRatio;
		//return latency* lat_ratio;
		return latency;
	...
	float BaseScheduler::getLatency(std::string device, const int model_num, const int batch, const NodePtr self_node, SimState &input){
		...
		float BaseScheduler::getLatency_new(std::string device, const int model_num, const int batch, const NodePtr self_node, SimState &input){
		assert(!device.empty());
		assert(batch>=1);
		float pure_latency = _nametoDevPerfModelTable[device].getLatency_new(_IDtoModelName[model_num], batch, self_node->resource_pntg);
		float interference_overhead=0.0;
		if(_useInterference){
			// do not try to get interference of non-existent GPU
			if (input.vGPUList.size() > self_node->id){ 
				GPUPtr self_gpu= input.vGPUList[self_node->id];
				double interference_overhead=getInterference_new(device,model_num,batch,self_node,self_gpu);
			}
		}
		return pure_latency+ interference_overhead;
	}
...
int BaseScheduler::getNewBatch(Task &self_task, const NodePtr &self_node, SimState &input, int &req, int &workload, int batch, const bool is_residue, bool interference){
#ifdef SCHED_DEBUG
		printNodeInfo(self_node);
#endif
		int batch_temp=0;
		...
		bool check_and_exit=false;
		bool check_last=false;
		while(high_batch>=low_batch){
			int mid_batch = floor((high_batch + low_batch)/2);
			if(interference)
				latency = getLatency_new(self_node->type,model_num,mid_batch,self_node, input);
			else
				latency = getLatency_new(self_node->type,model_num,mid_batch,self_node->resource_pntg);
			int throughput=floor(mid_batch*1000/latency);
			int num=ceil((workload+1)/mid_batch);
			int index=workload;
			bool slo_flag=true;
			for(int i=0;i<num;i++){
				float wait=0;
				if(index+1<(i+1)*mid_batch){
					wait=1000*((i+1)*mid_batch -index + 1)/req;
				}
				if (wait+latency>SLO){
					slo_flag=false;
					break;
				}
				//time = end_time;  //time update
			}
			if(slo_flag==false ){  //  add memory check ||
					if (throughput<req){
						return 0;
					}	
					high_batch=mid_batch;
			}
			else{    //satisfy SLO and memory ,consider throuput
				if(throughput>req){   //meet all constraits
					batch_temp=mid_batch;   //record the value
				}
				low_batch=mid_batch;
			}
		}
...
#ifdef SCHED_DEBUG
		std::cout << "[getNewBatch] original ret_batch is " << batch_temp << std::endl;
#endif
		
#ifdef SCHED_DEBUG
		std::cout << __func__ << ": returning bew_batch: " << batch_temp << std::endl; 
	...
	float BaseScheduler::getBatchLatency(std::string modelname, int batch){
		...
		else if(modelname == "resnet50" || modelname == "resnet18" || modelname == "vgg16" || modelname == "alexnet" || modelname == "mobilenet_v3" || modelname=="mnasnet" || modelname=="vgg19" || modelname=="efficientnet"){
