...
// stores value for saturating throughput of each available partition
		...
		for(auto type_num_pair : _typeToNumofTypeTable){
			...
			for(auto part : _availParts){
				...
				if (batch_size ==0){
					...
					float latency = getLatency_new(type,task.id,1,temp_node_ptr->resource_pntg);
				...
				float latency = getLatency_new(type,task.id,batch_size,temp_node_ptr->resource_pntg);
	...
	void IncrementalScheduler::estimateTrp(std::string device, Task &task, int rate, std::vector<NodePtr> &output_vec, const int MAX_PART){
		...
		if(limit < rate){
			...
			temp_node_ptr->duty_cycle = getLatency_new(device, task.id,sat_batch,temp_node_ptr->resource_pntg);
			...
			float latency = getLatency_new(device, task.id,max_batch,min_part);
			printf("!!!!!!!!!!!!!!!!!latency%lf",latency);
#ifdef SCHED_DEBUG
					...
					for(auto node_ptr: gpu_ptr->vNodeList){
						...
						if(node_ptr->vTaskList.empty()) target_nodes.push_back(node_ptr);
							...
							for(auto task_ptr: node_ptr->vTaskList){
								...
								latency_sum += getLatency_new(node_ptr->type,task_ptr->id,task_ptr->batch_size,node_ptr,decision);
							}
		...
		while(task.request_rate + task.additional_rate> TRP_SLACK){
			...
			for(auto node_ptr : candidates)
			{
				...
				for(local_batch_size = max_batch; local_batch_size > 0; --local_batch_size){
					...
					latency = getLatency_new(node_ptr->type,task.id,local_batch_size,node_ptr, sim);
				...
				for(auto task_ptr : node_ptr->vTaskList){
					...
					latency_sum += getLatency_new(node_ptr->type,task_ptr->id, batch_size, node_ptr, sim);
				...
				for(auto task_ptr : node_ptr->vTaskList){
					if(task_ptr->id == task.id){
						...
						new_occupancy += getLatency_new(node_ptr->type,task_ptr->id,task_ptr->batch_size,node_ptr,sim) / duty_cycle;
#ifdef SCHED_DEBUG
	...
	void IncrementalScheduler::getEstimateTrpST(std::string device, const Task &task, int rate, std::vector<NodePtr> &output_vec, const int MAX_PART)
		{
			...
			while (_rate > 0)
			{
				...
				float latency = getLatency_new(device, task.id, max_batch, max_part);
#ifdef SCHED_DEBUG
...
int IncrementalScheduler::getMinPart(std::string device, Task task, const NodePtr node_ptr, int &residue_rate ,int &result_batch){
		...
		for(auto part : _availParts){ // starting from highest partition
			...
			float latency = getLatency_new(device, task.id,max_batch,part);
	...
	// readjusting algorithm: migrates tasks and changes partition
		...
		bool IncrementalScheduler::readjust(Task &task, std::vector<NodePtr> &given, SimState &decision){
			...
			while(task.request_rate+task.additional_rate> TRP_SLACK){
				...
				float latency = getLatency_new(temp_node_ptr->type,task.id,max_batch,temp_node_ptr,decision);
				...
				if(!doesFitMemLimit(decision.vGPUList[temp_node_ptr->id],task.id,temp_node_ptr)){
					...
					temp_node_ptr->duty_cycle = getLatency_new(temp_node_ptr->type,task.id,max_batch,temp_node_ptr,decision);
			...
			if(proceed_to_residue){
				...
				//NodePtr temp_node_ptr = given.front();
					...
					float latency = getLatency_new(temp_node_ptr->type,task.id,temp_task_ptr->batch_size,temp_node_ptr,decision);
					temp_node_ptr->occupancy=latency / temp_node_ptr->duty_cycle;
#ifdef SCALE_DEBUG
	...
	bool IncrementalScheduler::findBestFit(SimState &input_sim,NodePtr &input,std::vector<NodePtr> &exclude_vec,NodePtr &output){
			...
			for (auto gpu_ptr : input_sim.vGPUList)
			{
				...
				for (auto node_ptr : gpu_ptr->vNodeList)
				{
					...
					float latency = getLatency_new(type,task_ptr->id,task_ptr->batch_size,node_ptr, input_sim);
	...
	bool IncrementalScheduler::allocateFit(const std::vector<NodePtr> &estimate, Task &task, SimState &decision){
			...
			for (auto input_ptr : estimate)
			{
				...
				if(best_pntg != 100 && best_pntg > input_pntg){
					...
					float latency = getLatency_new(input_ptr->type,temp->id,temp->batch_size,input_ptr,decision);
					if(!isresidue){
			...
			for(auto gpu_ptr : input_sim.vGPUList){
				for(auto node_ptr : gpu_ptr->vNodeList){
					...
					if(node_ptr->vTaskList.empty() && node_ptr->type == type_part_pair.first && node_ptr->resource_pntg ==type_part_pair.second){
						...
						node_ptr->occupancy = getLatency_new(node_ptr->type,temp_task_ptr->id,temp_task_ptr->batch_size,node_ptr,input_sim) / node_ptr->duty_cycle;
	...
	bool IncrementalScheduler::checkForInterference(std::string device, NodePtr the_node, NodePtr the_real_node, SimState &sim){
			...
			if(!getOtherNode(the_real_node,neighbor_node,sim)){
				for(auto task_ptr : neighbor_node->vTaskList){
					...
					float pure_latency = _nametoDevPerfModelTable[device].getLatency_new(getModelName(task_ptr->id),task_ptr->batch_size,neighbor_node->resource_pntg);
