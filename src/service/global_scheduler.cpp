...
extern std::map<proxy_info*, std::deque<std::shared_ptr<batched_request>>* > PerProxyOutputQueue;
extern std::map<proxy_info*, int > PerProxyNum;
...
#endif*/
	if (decision.size()==0){
		//printf("[SCHEDULER]:decision=0");
		return;
	} 

	for(unsigned int i =0; i < decision.size(); i++){
		printf("[SCHEDULER]:decision=i");
		proxy_info* pPInfo = decision[i]->proxy;
...
std::vector<std::shared_ptr<TaskSpec>> GlobalScheduler::executeScheduler(SysMonitor *SysState){
	...
	switch(mSchedulerMode){
		...
		case ORACLE:
			...
			break;
		case NEW_SCHEDULER:
			decision = NewScheduler(SysState);
			break;
		case DRL_SCHEDULER:
			//decision = DRLScheduler(SysState);
			break;
		case GLET_SCHEDULER:
			decision = GletScheduler(SysState);
			break;
...
void GlobalScheduler::initChangeBatch(std::string ReqName){
	if (PerTaskBatchUpdateMtx.find(ReqName) == PerTaskBatchUpdateMtx.end()){
		PerTaskBatchUpdateMtx[ReqName] = new std::mutex();
		mPerTaskChangeBatch[ReqName]=false;
	}
}

bool GlobalScheduler::getTaskChangeBatch(std::string model_name){
	return mPerTaskChangeBatch[model_name];
}

void GlobalScheduler::updateChangeBatch(std::string ReqName, bool change){
	PerTaskBatchUpdateMtx[ReqName]->lock();
	mPerTaskChangeBatch[ReqName]=change;
	PerTaskBatchUpdateMtx[ReqName]->unlock();
}
...
void GlobalScheduler::setupModelBatch(std::string name, int batch_size)
{
	mPermodelBatch[name]=batch_size;
}

int GlobalScheduler::getModelBatch(std::string name)
{
	return mPermodelBatch[name];
}


void GlobalScheduler::setupModelWorkload(std::string name, int batch_num)
{
	mPerModelWorkload[name]=batch_num;
}

int GlobalScheduler::getModelWorkload(std::string name)
{
	return mPerModelWorkload[name];
}
...
int GlobalScheduler::getPlace(std::string name){
	return mPerModelPlace[name];
}
		...
		for(auto pPInfo : *SysState->getDevMPSInfo(j)){
			...
			if(SysState->getSizeofBatchQueueofProxy(pPInfo) == 0){
				...
				for(auto pair : *SysState->getProxyNetList(pPInfo)){
					...
					pTSpec->device_id=j;
					/*
					int batch_size=getModelBatch(pair.first);
					if (batch_size!=pair.second){
						pTSpec->BatchSize=batch_size;
					}
					else{
...
float GlobalScheduler::getEstLatency_new(std::string task, int batch, int cap, std::string type){
	return  SBPScheduler.getLatency_new(type,getModelID(task),batch,cap);
}
...
void GlobalScheduler::setupTasks_1(SysMonitor *SysState, bool include_downsize, std::vector<Task> &output_vec){
	std::vector<std::string> tasks;
...
void GlobalScheduler::setupTasks(SysMonitor *SysState, bool include_downsize, std::vector<Task> &output_vec){
	std::vector<std::string> tasks;
	const float UP_THRESHOLD=8.0; // upper threshold of increasing rate, in percentage
	const int UP_THRESHOLD_RATE = 8; // upper threshold of increasing rate, in absolute rate
	const float DOWN_THRESHOLD=8.0; // threshold for lowering target range, in percentage
	const int DOWN_THRESHOLD_RATE = 8; // upper threshold of increasing rate, in absolute rate
	const float RATE_ROOM = 1;//1.2;
	const float AGGRESIVE_RATE_ROOM=1;//1.5;
	//for (std::map<std::string,std::queue<std::shared_ptr<request>>>::iterator it = SysState->ReqListHashTable.begin(); it != SysState->ReqListHashTable.end(); it++){
	for(auto it = SysState->getVectorOfNetNames()->begin(); it != SysState->getVectorOfNetNames()->end(); it++){
		// if target was not initiated at all and rate is 0, then continue
		float curr_rate = getTaskArivRate(*it);
		std::cout << "[setuptasks] task_name:" << *it << ", curr_rate: " <<curr_rate<< std::endl;
		if (curr_rate ==0 && (mPerTaskTgtRate.find(*it) == mPerTaskTgtRate.end()) ) continue;
		float org_rate = curr_rate; //updateAndReturnMaxHistRate(*it,curr_rate);
		float tgt_rate;
		if(org_rate < 125){
			tgt_rate = org_rate * AGGRESIVE_RATE_ROOM;
		}
		...
		std::cout << "[setuptasks] task_name:" << *it << ", mPerTaskTgtRate: " <<mPerTaskTgtRate[*it]<< std::endl;
		if ( mPerTaskTgtRate[*it]==0) continue;
		// if target was not initiated at all, initiate targeting rate andd 
		if(mPerTaskTgtRate.find(*it) == mPerTaskTgtRate.end()){
			mPerTaskTgtRate[*it] =tgt_rate;
#ifdef EPOCH_DEBUG
			std::cout << "[setuptasks] not find task_name" << *it<<std::endl;
#endif
			tasks.push_back(*it);
		}
		else{
			...
			float diff_rate = tgt_rate - mPerTaskTgtRate[*it];
			#ifdef EPOCH_DEBUG
			std::cout << "[setuptasks] task_name:" << *it << ", diff_pntg: " <<diff_pntg<<", diff_rate :" <<diff_rate << std::endl;
#endif
			if( diff_pntg > UP_THRESHOLD || diff_rate > UP_THRESHOLD_RATE){
				mPerTaskTgtRate[*it] = tgt_rate;
#ifdef EPOCH_DEBUG
			std::cout << "[setuptasks] up  task_name" << *it<<std::endl;
#endif
				tasks.push_back(*it);
			}
...
#ifdef EPOCH_DEBUG
				std::cout << "[setuptasks] SYS_FLUSH DETECTED" << std::endl;
				std::cout << "[setuptasks] down  task_name" << *it<<std::endl;
#endif
...
void GlobalScheduler::updateGPUMemState(SimState &input, SysMonitor *SysState, int num_of_gpus_to_update){
	...
	std::cout<< input.vGPUList.size() <<">=" <<num_of_gpus_to_update<<std::endl;
...
std::vector<std::shared_ptr<TaskSpec>> GlobalScheduler::OracleScheduler(SysMonitor *SysState){
	...
	if((now-last_epoch) / 1000000 > EPOCH_INTERVAL){
		...
		p1=getCurNs();
		setupTasks_1(SysState,true,session);
#ifdef EPOCH_DEBUG
...
/*
std::pair<bool,int> GlobalScheduler::adjustbatchsize(std::string model_name, float rate, float workload, double workload_wait, const NodePtr node_ptr, SimState decision){
	int model_num = SBPScheduler.getModelID(model_name);
	bool flag = true;
	int batch;
	int start = 32;
	int end = 1;
	while(end<=start){
		int b = end + (start-end)/2;
		float latency = getLatency_new(node_ptr->type,model_num, b , node_ptr, decision);
		float throughput = 1000/latency;
		num


	} 

	return std::make_pair(flag, batch);
}
*/

std::vector<std::shared_ptr<TaskSpec>> GlobalScheduler::NewScheduler(SysMonitor *SysState){
	static uint64_t last_epoch=0;
	bool flush= false; // flag indicating whether to force flush or not
	std::vector<std::shared_ptr<TaskSpec>> decision;
	if (last_epoch ==0) last_epoch = getCurNs(); // this only occurs once
	...
	if((now-last_epoch) / 1000000 > EPOCH_INTERVAL){
		...
		if(mNeedUpdate){
			...
			mNeedUpdate=false;
			// the following unloadModels is asynchronous call
			//std::cout << "---after  update: prev_state---"  << std::endl;
		...
		std::map<std::string, int> ModelPlace;
		std::map<std::string, int> ModelRes;
		std::map<int, int>GPUUsedmemory;
		...
		if(!session.empty()){
			printf("session length: %lu \n", session.size());
			for (auto task : session){
				printf("task: %d, arriving_rate: %lf \n", task.id, mPerTaskTgtRate[SBPScheduler.getModelName(task.id)]);
			}
		}
#endif
		if(!session.empty()){
#ifdef EPOCH_DEBUG
			printf("[NewScheduler] (epoch) called %s \n", timeStamp() );
#endif
/*
			std::vector<std::string> tasks;
			for (auto task:session){
				tasks.push_back(SBPScheduler.getModelName(task->id));
			}
*/
			sort(session.begin(), session.end(), cmp_task_rate_dsc);
			copySession(session,backup_session);
			...
			while(!success){
				...
				for(auto gpu_ptr : prev_output.vGPUList){
					for(auto iter1 : gpu_ptr->vNodeList){         
						if(!iter1->vTaskList.empty()) {
							...
							//int batch_num_1 = PerProxyOutputQueue[pPInfo]->size();
							int batch_num=PerProxyNum[pPInfo];
							//SysState.PerProxyOutputQueue[pPInfo]->size();
							for(auto task_ptr : iter1->vTaskList){
								std::string model_name=SBPScheduler.getModelName(task_ptr->id);
								ModelPlace[model_name]=iter1->id;
								ModelRes[model_name]=iter1->resource_pntg;
								setupModelWorkload(model_name, batch_num);
								int workload=SysState->getLenofReqQueue(model_name);
								printf("!!!![NewScheduler] model:%s, batch:%d,  place:%d,  res:%d,workload:%d\n", model_name.c_str(),batch_num,iter1->id,iter1->resource_pntg,workload);
							}
						}
					}
					GPUUsedmemory[gpu_ptr->GPUID]=gpu_ptr->used_memory;
				}

				std::string result = "../resource/algorithm/result.txt";
				struct stat file_stat;
				time_t last_time;
				if(stat(result.c_str(), &file_stat) == -1) {
        		    // 文件不存在或者读取出错
						last_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
        				break;
    			}
				last_time = file_stat.st_mtime;
				
				FILE * file_write= fopen("../resource/algorithm/state.txt", "w");
				//for (auto task:session){
				for(auto it = SysState->getVectorOfNetNames()->begin(); it != SysState->getVectorOfNetNames()->end(); it++){
					std::string model_name=*it;
					float rate= getTaskArivRate(*it);
					if (rate==0){continue;}
					int workload=SysState->getLenofReqQueue(model_name);
					double workload_wait = 0;
					if (workload>0){
						workload_wait = double(getCurNs() -  SysState->getRequestQueueofNet(model_name)->front()->getStart())/1000000;
						printf("workload_wait : %lf\n",workload_wait);
					}
					int place=ModelPlace[model_name];
					int res=ModelRes[model_name];
					int batch=getModelBatch(model_name);
					printf("!!!![NewScheduler] model:%s, batch:%d, rate:%f; workload:%d, workload_wait : %lf, place:%d,res:%d\n", model_name.c_str(),batch,rate,workload,workload_wait, place,res);
					 fprintf(file_write,"%s,%lf,%d,%lf,%d,%d,%d\n", model_name.c_str(), rate,workload, workload_wait,place,res,batch);
 
				}
				printf("used_memory:%d,    2 :%d",GPUUsedmemory[0], GPUUsedmemory[1]);
				fprintf(file_write,"used_memory,");
				for(auto gpu_ptr : prev_output.vGPUList){
					fprintf(file_write,"%d,", GPUUsedmemory[gpu_ptr->GPUID]);
				}

				fflush(file_write);
				uint64_t t_1 = getCurNs();
				...
				uint64_t t_2 = getCurNs();
				std::string str_buf;
                std::fstream fs;
                fs.open("../resource/algorithm/wait.txt", std::ios::in);

                getline(fs, str_buf);
                wait = stoi(str_buf);

				printf("New scheduling time: %lf,  wait_flag = %d\n", double(t_2-t_1)/1000000,wait);

				std::string filename="../resource/algorithm/result.txt";
				//"../resource/proxy/result.txt";
				if(readAndTrans(filename,SysState,output)){
#ifdef SCALE
					int curr_gpus = SBPScheduler.getMaxGPUs();
					// check if we can get more GPUs
					if(curr_gpus < SysState->getNGPUs()){// if so, add more gpus and continue
...
#ifdef EPOCH_DEBUG
			uint64_t end = getCurNs();
			printf("[NewScheduler] SYNC took %lf ms \n", double(p1-start)/1000000);
			printf("[NewScheduler] scheduling took %lf ms \n", double(end-start)/1000000 );
#endif
		} // session not empty
		last_epoch=getCurNs();
	}
...
std::vector<std::shared_ptr<TaskSpec>> GlobalScheduler::GletScheduler(SysMonitor *SysState){
	static uint64_t last_epoch=0;
	bool flush= false; // flag indicating whether to force flush or not
	std::vector<std::shared_ptr<TaskSpec>> decision;
	if (last_epoch ==0) last_epoch = getCurNs(); // this only occurs once
	...
	if((now-last_epoch) / 1000000 > EPOCH_INTERVAL){
		...
		if(!session.empty()){
			...
			while(!success){
				...
				FILE * file_write= fopen("../resource/rates.csv", "w");
				fprintf(file_write,"6,\n");
				/*
				for (auto task:session){
					fprintf(file_write,"%d,%d,%d,\n",task.id,task.request_rate,task.SLO);
				}
				*/
				for(auto it = SysState->getVectorOfNetNames()->begin(); it != SysState->getVectorOfNetNames()->end(); it++){
					std::string model_name=*it;
					float rate= getTaskArivRate(model_name);
					int model_rate = rate;
					std::cout<<"model_name:"<<model_name<<", rate:"<<model_rate<<std::endl;
					if (rate!=0){
						fprintf(file_write,"%d,%d,%d,\n",SBPScheduler.getModelID(model_name),model_rate,getSLO(model_name));
					}
				}
				...
				uint64_t t_1 = getCurNs();
				std::string shell_cmd ="../scripts/executeScheduler_example.sh ->scheduling.txt";
				system(shell_cmd.c_str());
				uint64_t t_3 = getCurNs();
				printf("Run scheduling time: %lf\n", double(t_3 - t_1)/1000000);
				

				std::string filename="../resource/ModelList_1.txt";
				//"../resource/proxy/result.txt";
				if(readAndTrans(filename,SysState,output)){
#ifdef SCALE
					int curr_gpus = SBPScheduler.getMaxGPUs();
...
int GlobalScheduler::onereadAndBoot(SysMonitor *SysState, std::string model_list_file){
#ifdef SCHED_DEBUG
	...
	// setup tasks
	int num_gpu=1;
	flushandUpdateDevs(mPrevState,num_gpu);
	bool flag=readAndTrans(model_list_file,SysState,mCurrState);
...
void GlobalScheduler::flushandUpdateDevs(SimState &input, const int num_of_gpus){
	...
	std::cout<<"device num:"<<input.vGPUList.size()<<std::endl;
