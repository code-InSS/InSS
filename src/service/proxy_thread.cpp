...
std::map<proxy_info*, int > PerProxyNum;
std::map<proxy_info*, std::mutex*> PerProxyNumMtx;
...
void*  initProxy(void *args){
	...
	for (int i =0; i<3 ; i++){ // we have three cores per each GPU
		T(gpucores[DeviceID][i], &set
		printf("core %d set for PROXY[%d,%d,%d] \n",gpucores[DeviceID][i],pPInfo->dev_id, pPInfo->cap, pPInfo->dedup_num);
	}
	if (pthread_setaffinity_np(thread, sizeof(t_t), &set0){
		printf("Error in setting affinity for read\n");
		exit(1);
	}
	...
	while (1){
		...
		while((pBatchedReq->getBatchNum() < pBatchedReq->getMaxBatch()))
		{
			...
			while(!pReqList->empty()){
				std::shared_ptr<request> r= pReqList->front();
				pReqList->pop();
				uint64_t now=getCurNs();
				uint64_t start = r->getStart();
						// compare with SLO
				if(double(now-start)/1000000 > maxdelay){
					r->setDropped(true);
					if(!r->getApp()->isDropped()){
						r->getApp()->setDropped(true);
						addtoAppQueue(r->getApp(), /*dropped=*/true);
					}
					ServerState.getCompQueue()->enqueue(r);
				}else{
					ReqMtxVec[StrReqName]->unlock();
...
void ProxyBasicSetup(proxy_info* pPInfo){
	...
	PerProxyNumMtx[pPInfo]=new std::mutex();
	...
	proxyname = proxyname + std::to_string(pPInfo->dev_id)+std::to_string(pPInfo->cap)+std::to_string(pPInfo->dedup_num);
	...
	//std::cout <<"!!!!!nets:"<< pVec->size() <<std::endl;
...
void* sendBatchedRequest(std::shared_ptr<batched_request> &input_info, std::string strReqName, std::shared_ptr<TaskSpec> pTask, proxy_info* pPInfo) {
...
#ifdef DEV_LOG
	std::string msg=std::to_string(tnum);
	...
	PerProxyNumMtx[pPInfo]->lock();
	PerProxyNum[pPInfo]++;
	PerProxyNumMtx[pPInfo]->unlock();
	...
	uint64_t end_send=getCurNs();
	printf("[DEBUG] sent tasks: %u, sending overhead:%lf \n", sent_tasks,double(end_send-start_send)/1000000);
	debug_mtx.unlock();    
#endif
