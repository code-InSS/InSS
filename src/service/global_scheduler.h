...
class GlobalScheduler
{
	...
	public: 
		...
		std::vector<std::shared_ptr<TaskSpec>> NewScheduler(SysMonitor *SysState);
		std::vector<std::shared_ptr<TaskSpec>> GletScheduler(SysMonitor *SysState);
		std::vector<std::shared_ptr<TaskSpec>> DRLScheduler(SysMonitor *SysState);
		...
		void setupSBPScheduler(SysMonitor *SysState, std::map<std::string,std::string> &param_file_list, std::string res_dir, int flag);
		...
		void setupModelPlace(std::string name, int Place);
		...
		bool readAndTrans(std::string specfile, SysMonitor *SysState, SimState &output);

		//update batch decision
		void initChangeBatch(std::string ReqName);
		bool getTaskChangeBatch(std::string model_name);
		void updateChangeBatch(std::string ReqName, bool change);
		...
		void setupTasks_1(SysMonitor *SysState, bool include_downsize, std::vector<Task> &output_vec);
		...
		//std::pair<bool,int> adjustbatchsize(std::string model_name, float rate, float workload, double workload_wait, const NodePtr self_node, SimState decision);
		...
		int getPlace(std::string name);
		float getEstLatency(std::string task, int batch, int cap, std::string type);
		float getEstLatency_new(std::string task, int batch, int cap, std::string type);
		...
		void setupModelBatch(std::string name, int batch_size);
		int getModelBatch(std::string name);
		void setupModelWorkload(std::string name, int batch_num);
		int getModelWorkload(std::string name);
		...
		int onereadAndBoot(SysMonitor *SysState, std::string model_list_file);
	...
	private: 
		...
		/*used for scheduling update*/
		std::map<std::string, bool> mPerTaskChangeBatch;//used for batch update
		std::map<std::string, std::mutex*> PerTaskBatchUpdateMtx; 
		std::map<std::string, int> mPermodelBatch;

		InterferenceModeling::InterferenceModel mInterferenceModel;   
		LatencyModel mLatencyModel;
		std::map<std::string,int> mPerModelSLO;
		std::map<std::string,int> mPerModelPlace;
		std::map<std::string,int> mPerModelWorkload;
