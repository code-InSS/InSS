...
void* initServer(void* numGPU){
	...
	while (1){
		...
		for(std::vector<std::string>::iterator it = gScheduler.getNetNames()->begin();  it != gScheduler.getNetNames()->end(); it++){
			if (gScheduler.getTaskChangeBatch(*it)){
				batchinit=(batchinit)%4+1;
				gScheduler.UpdateBatchSize(&ServerState, *it, batchinit);
				gScheduler.updateChangeBatch(*it,false);
				//int batch_size = gScheduler.getModelBatch(*it);
				
				//int new_batch_size = (batch_size+1)%2+1;
				gScheduler.setupModelBatch(*it, batchinit);
			} 
		}
		*/
