...
int GlobalScheduler::setSchedulingMode(std::string mode, SysMonitor *SysState){
	...
	if(mode == "mps_static"){
		...
		mSchedulerMode=NEW_SCHEDULER;
		ALLOW_REPART=true;
	}
	else if(mode == "glet"){
		mSchedulerMode=GLET_SCHEDULER;
		ALLOW_REPART=true;
	}
	else if(mode == "drl"){
		mSchedulerMode=DRL_SCHEDULER;
		ALLOW_REPART=true;
	}
	else
		return EXIT_FAILURE;
...
void GlobalScheduler::setupSBPScheduler(SysMonitor *SysState, std::map<std::string,std::string> &param_file_list, std::string res_dir,int flag){
	setupPerModelLatencyVec(SysState);
	//for the moment we hardcode the name of files required
	std::vector<std::string> files = {"3_224_224.txt"};//{"1_28_28.txt", "3_224_224.txt", "3_300_300.txt"};
...
void GlobalScheduler::setupScheduler(SysMonitor* SysState, std::map<std::string,std::string> &param_file_list, std::string scheduler, std::string res_dir ){
	...
	switch(mSchedulerMode){
		case MPS_STATIC:
			setupSBPScheduler(SysState,param_file_list,res_dir, 0);
			//printf("ok");
		...
		case ORACLE:
			setupSBPScheduler(SysState,param_file_list, res_dir, 0);
			EPOCH_INTERVAL= 2000; // interval of checking rates, in ms,
			BEACON_INTERVAL=EPOCH_INTERVAL; // required in initReqRate
			HIST_LEN=30 / float(EPOCH_INTERVAL / 1000.0);
			break;
		case NEW_SCHEDULER:
			setupSBPScheduler(SysState,param_file_list, res_dir, 0);
			EPOCH_INTERVAL= 3000; // interval of checking rates, in ms,
			BEACON_INTERVAL=EPOCH_INTERVAL; // required in initReqRate
			HIST_LEN=30 / float(EPOCH_INTERVAL / 1000.0);
			break;
		case GLET_SCHEDULER:
			...
			break;
		case DRL_SCHEDULER:
			setupSBPScheduler(SysState,param_file_list, res_dir, 1);
			//add code to cmd run python get output of actor network
			EPOCH_INTERVAL= 3000; // interval of checking rates, in ms,
			BEACON_INTERVAL=EPOCH_INTERVAL; // required in initReqRate
			HIST_LEN=30 / float(EPOCH_INTERVAL / 1000.0);
			break;
...
void GlobalScheduler::createProxyInfos(SysMonitor *SysState, std::vector<int> parts, int node_id, int ngpus, std::string type){
	...
	for(int devindex = SysState->getNGPUs(); devindex < SysState->getNGPUs()+ngpus; devindex++ ){
		//std::cout << __func__ << ": devindex = " << devindex << std::endl;
		for(int part: parts){
			//std::cout << __func__ << ": part = " << part << std::endl;
			int dedup_num=0;
...
void GlobalScheduler::setupInitTgtRate(std::string rate_file){
	...
	{ 
		...
		std::cout << id<<","<< request_rate<<"," << SLO<<"," << SBPScheduler.getModelName(id) << std::endl;
	}  
	fs.close();
#ifdef EPOCH_DEBUG
...
void GlobalScheduler::setupMPSInfo(SysMonitor *SysState, std::string capfile){
	...
	while(getline(infile,line)){
		...
		std::cout << "------------------------------------------------"<<cap << " and "<< dedup_num<< found << std::endl; 
		if(!found){
		...
		std::cout << "use"<< cap << " and "<< dedup_num << std::endl;
	}
...
bool GlobalScheduler::readAndTrans(std::string specfile, SysMonitor *SysState, SimState &output){
#ifdef FRONTEND_DEBUG
	printf("!!!!!!!![setup] readAndTrans called for file: %s \n", specfile.c_str());
#endif 
	//throughput info to deliver to load balancer
	//std::map<proxy_info*, std::vector<std::pair<int,double>>> mapping_trps;
	int num_gpu=SysState->getNGPUs();
	assert(SysState->getNGPUs()>=1);
	printf("GPU_nums: %d", num_gpu);
	flushandUpdateDevs(output,num_gpu);
	...
	for(auto iter1 : output.vGPUList){
		iter1->vNodeList.clear();
	}
	for(std::vector<task_config>::iterator it = list.begin(); it != list.end(); it++){
		if(it->name=="reserved"){
			continue;
		}
		setupModelPlace(it->name,it->node_id);
		Node new_node;
		NodePtr new_node_ptr=std::make_shared<Node>(new_node);
		new_node_ptr->id = it->node_id;
		new_node_ptr->resource_pntg = it->thread_cap;
		new_node_ptr->dedup_num = it->dedup_num;
		new_node_ptr->duty_cycle = it->duty_cycle;
		new_node_ptr->reserved = false;
		//new_node_ptr->occupancy = node_ptr->occupancy;
		new_node_ptr->type = output.vGPUList[it->node_id]->TYPE;
		Task new_task;
		TaskPtr new_task_ptr= std::make_shared<Task>(new_task);
		int model_id = getModelID(it->name);
		new_task_ptr->id=model_id;
		new_task_ptr->batch_size=it->batch_size;
		new_task_ptr->request_rate=mPerTaskTgtRate[it->name];
		new_task_ptr->SLO=getSLO(it->name);
		new_task_ptr->throughput=it->batch_size * (1000 / it->duty_cycle);
		new_node_ptr->vTaskList.push_back(new_task_ptr);
		output.vGPUList[it->node_id]->vNodeList.push_back(new_node_ptr);
	}
	return EXIT_SUCCESS;
}
...
void GlobalScheduler::setupProxyTaskList(std::string specfile, SysMonitor *SysState){
	...
	std::cout <<"type " << ptemp->type <<",[setup] name: " << it->name << ", batch_size: " << it->batch_size << std::endl;
#ifdef DEBUG
		...
		setupModelBatch(it->name,it->batch_size);
...
void GlobalScheduler::setupModelPlace(std::string name, int place)
{
	mPerModelPlace[name]=place;
#ifdef DEBUG
	printf("[SETUP] mPerModelPlace[%s]: %d \n", name.c_str(), place);
#endif
}
