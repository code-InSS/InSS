...
void initDropThread(po::variables_map &vm, SysMonitor &state){
    ...
    printf("drop_ratio: %.2f",drop_ratio);
    assert(0<= drop_ratio);
...
int configureProxys(BackendDelegate *backend_del, int node_id, SysMonitor *SysState){
  ...
  << std::endl;
  #endif
  //std::cout << "!!!!!!!!!!!!!!!!!!"<<__func__ << backend_del->getType()<< node_id;
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "nets:"<<gScheduler.getNetNames()->size() << std::endl;
  ...
  else if(vm["scheduler"].as<std::string>() == "new"  ){//||vm["scheduler"].as<std::string>() == "oracle"
    gScheduler.onereadAndBoot(&ServerState, vm["model_list"].as<std::string>());
  }
  else{
