...
def sysprocess(kind,kernel):
    values=[0] * 5
    orginal=[0] * 5
    pathb="data/2060s_sysidletime_"
    for i in range(1,6):
        path=pathb+str(i)+".sqlite"
        ans=conse(path)
        idlep=sepper(path,str(ans[0]),str(ans[1]))
        dur=runtime(path,str(ans[0]),str(ans[1]),kernel)
        orginal[i - 1] = multimean(idlep, dur)
        values[i-1]=multimean(idlep,dur)/kernel
    print(values)
    saverecords(kind,"idletime",orginal)
    saverecords("handware", "idletime", values)
...
if __name__ == '__main__':
    ...
    kernels = initcontext()
    #sysprocess(sys.argv[2],kernels[sys.argv[2]])
    if(sys.argv[1]=="sys"):
        sysprocess(sys.argv[2],kernels[sys.argv[2]])#sysprocess()
    elif(sys.argv[3]=="1"):
